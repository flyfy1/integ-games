...
export const platformer: GameModule = {
  ...
  mount(host: HTMLElement, services: GameServices): GameController {
    ...
    let raf = 0, paused = false, won = false, dead = false, x = 32, y = 458, vx = 0, vy = 0, stage = 1, elapsed = 0;
    const reset = () => { paused = false; won = false; dead = false; x = 32; y = 458; vx = 0; vy = 0; stage = 1; elapsed = 0; };
    const jump = () => { if (!paused && !won && !dead && vy === 0) { vy = -11; services.sound.play('move'); } };
    ...
    const loop = () => { if (!paused && !dead && !won) { elapsed++; if (k.keys.has('ArrowLeft') || k.keys.has('a')) vx = -4; if (k.keys.has('ArrowRight') || k.keys.has('d')) vx = 4; x = clamp(x + vx, 0, 342); vy += .55; y += vy; let landed = false; for (const platform of platforms) if (vy >= 0 && x + 16 > platform.x && x < platform.x + platform.width && y + 26 >= platform.y && y + 26 <= platform.y + 18) { y = platform.y - 26; vy = 0; landed = true; } if (!landed && y > 540) { dead = true; services.sound.play('fail'); } for (const spike of spikes) if (x + 16 > spike.x && x < spike.x + spike.width && y + 26 > spike.y - 13 && y < spike.y + 4) { dead = true; services.sound.play('fail'); }
      if (x > 272 && x < 315 && y < 260 && y > 205) { won = true; stage++; k.score(Math.max(10, 600 - Math.floor(elapsed / 3))); services.reportComplete(stage); services.sound.play('success'); } } draw(); raf = requestAnimationFrame(loop); };
