...
export const fruitMerge: GameModule = {
  ...
  mount(host: HTMLElement, services: GameServices): GameController {
    ...
    const reset = () => { paused = false; lost = false; score = 0; level = 1; held = 180; next = 0; fruits = []; k.fx.clear(); };
    const drop = () => { if (lost) { reset(); return; } if (!fruits.some(f => f.y < 85)) { fruits.push({ x: held, y: 62, vy: 0, level: next }); k.fx.burst(held, 62, colors[next], 4); next = Math.floor(services.random() * Math.min(4, level + 2)); services.sound.play('move'); } };
    ...
    const draw = () => { const c = k.ctx; k.clear(); c.fillStyle = '#1a1326'; c.fillRect(0, 0, 360, 560); c.strokeStyle = '#ff8f70'; c.lineWidth = 3; c.strokeRect(15, 42, 330, 500); text(c, `ORBIT MASS ${score}  ·  TIER ${level}`, 180, 27, 14, '#a8b1c5');
      ...
      k.fx.draw(); if (paused) text(c, 'PAUSED', 180, 280, 27); if (lost) { text(c, 'ORBIT OVERFLOW', 180, 260, 23, '#ff6b7a'); text(c, 'Tap or press Space to restart', 180, 290, 15); }
    };
