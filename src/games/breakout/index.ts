...
export const breakout: GameModule = {
  ...
  mount(host: HTMLElement, services: GameServices): GameController {
    ...
    const reset = () => { paused = false; over = false; score = 0; level = 1; paddle = 140; ball = { x: 180, y: 426, dx: 3.1, dy: -3.5 }; fill(); };
    ...
    const loop = () => { if (!paused && !over) { if (k.keys.has('ArrowLeft') || k.keys.has('a')) paddle = clamp(paddle - 6, 8, 272); if (k.keys.has('ArrowRight') || k.keys.has('d')) paddle = clamp(paddle + 6, 8, 272); ball.x += ball.dx; ball.y += ball.dy; if (ball.x < 6 || ball.x > 354) { ball.dx *= -1; services.sound.play('move'); } if (ball.y < 45) ball.dy = Math.abs(ball.dy); if (ball.y > 540) { over = true; services.sound.play('fail'); }
      if (ball.y > 492 && ball.y < 514 && ball.x > paddle - 5 && ball.x < paddle + 85 && ball.dy > 0) { ball.dy = -Math.abs(ball.dy); ball.dx = (ball.x - (paddle + 40)) / 12; services.sound.play('hit'); }
      for (const brick of [...bricks]) if (ball.x > brick.x - 5 && ball.x < brick.x + 41 && ball.y > brick.y - 5 && ball.y < brick.y + 23) { ball.dy *= -1; services.sound.play('hit'); if (--brick.hp === 0) { bricks.splice(bricks.indexOf(brick), 1); score += 10 * level; k.score(score); services.sound.play('clear'); } break; }
      if (!bricks.length) { level++; score += 100; k.score(score); ball = { x: 180, y: 426, dx: 3 + level * .15, dy: -3.4 - level * .12 }; fill(); services.reportComplete(level); services.sound.play('upgrade'); } } draw(); raf = requestAnimationFrame(loop); };
