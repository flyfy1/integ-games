...
export const game: GameModule = {
  ...
  mount(host, services) {
    ...
    const tick = () => { if (dead || paused) return; direction = queued; const head: [number, number] = [snake[0][0] + direction[0], snake[0][1] + direction[1]]; if (head[0] < 0 || head[0] >= cells || head[1] < 0 || head[1] >= cells || snake.some(([x, y]) => x === head[0] && y === head[1])) { dead = true; services.sound.play('fail'); const record = best(services, 'snake-best', score); services.reportScore(score); frame.setMessage(`Signal lost — ${score} points. Best ${record}. Press R to retry.`); return; } snake.unshift(head); if (head[0] === food[0] && head[1] === food[1]) { score += 10; food = spawn(); services.sound.play('collect'); services.reportScore(score); } else snake.pop(); draw(); };
