...
export const hexPuzzle: GameModule = {
  ...
  mount(host: HTMLElement, services: GameServices): GameController {
    ...
    const draw = () => { const c = k.ctx; k.clear(); c.fillStyle = '#14142b'; c.fillRect(0, 0, 360, 560); text(c, `FIT ${score}  ·  ROUND ${round}`, 180, 27, 14, '#a8b1c5'); cells.forEach(cell => { const point = pos(cell.q, cell.r); hex(point.x, point.y, cell.on ? '#8b7cff' : '#273149', cell.q === cursor.q && cell.r === cursor.r ? '#70f0c2' : undefined); });
      const preview = pieces[selected]; if (preview) preview.forEach(([q, r]) => { const point = pos(cursor.q + q, cursor.r + r); hex(point.x, point.y, canPlace(preview, cursor.q, cursor.r) ? '#70f0c255' : '#ff6b7a55'); }); for (let i = 0; i < 3; i++) { c.fillStyle = selected === i ? '#70f0c2' : '#273149'; c.fillRect(i * 120 + 8, 440, 104, 80); const shape = pieces[i]; if (shape) shape.forEach(([q, r]) => hex(i * 120 + 38 + q * 24, 470 + r * 21, '#ffcb6b')); text(c, String(i + 1), i * 120 + 102, 505, 12, '#a8b1c5'); } k.fx.draw(); if (paused) text(c, 'PAUSED', 180, 280, 27); if (gameover) { text(c, 'NO FITS LEFT', 180, 280, 24, '#ff6b7a'); text(c, 'Tap or press Space to restart', 180, 310, 14); } };
    const loop = () => { if (!paused) k.fx.step(); draw(); raf = requestAnimationFrame(loop); }; loop(); return { pause: () => { paused = true; }, resume: () => { paused = false; }, restart: reset, destroy: () => { cancelAnimationFrame(raf); k.dispose(); } };
  }
};
